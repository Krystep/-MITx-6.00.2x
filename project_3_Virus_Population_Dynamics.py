...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pop_list = np.zeros(300)
    
    for trial in range(numTrials):
        patient = Patient(viruses, maxPop)
        for step in range(300):
            change = patient.update()
            pop_list[step] += change
       
    avg_pop = pop_list/numTrials
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        """
        if newDrug not in self.list_of_drugs:
            self.list_of_drugs.append(newDrug)
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try: 
                new = virus.reproduce(popDensity, self.list_of_drugs)
                self.viruses.append(new)
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        for step in range(150):
            change = patient.update()
            pop_list[step] += change
            Resist_pop_list[step] = patient.getResistPop(['guttagonol'])
        ...
        for step in range(150, 300):
            ...
            Resist_pop_list[step] = patient.getResistPop(['guttagonol'])
    avg_pop = pop_list/numTrials
    Resist_pop = Resist_pop_list/numTrials

    pylab.plot(avg_pop, label = "ResistantVirus")
    pylab.plot(Resist_pop, label = "Resistant")
